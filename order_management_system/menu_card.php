<?php 
session_start();
include '../config/config.php';
include '../config/test_input.php';
$filterCategory = "";

if(isset($_GET['c'])){
    $filterCategory = " AND category_ID = '".$_GET['c']."'";
}

$keyword = "";
if (isset($_GET['k'])) {
    $temp = test_input($_GET['k']);
    $keyword = " AND food.name LIKE '%$temp%'";
}
?>
<div class="row">
    <?php
    $page =@ $_POST['page'];
	if ($page == 0 || $page == 1) {
		$page = 0;
	}else{
		$page = ($page * 9) - 9;
	}
    $sql = "SELECT username FROM stall WHERE ID = '".$_SESSION['stall_ID']."';";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result) == 1){
        $stall_username = mysqli_fetch_assoc($result)['username'];
    }
    $sql = "SELECT 
    food.ID AS ID, 
    food.name AS name, 
    category.name AS category, 
    category.ID as category_ID,
    image,
    price, 
    available 
    FROM food LEFT JOIN category ON food.category_ID = category.ID WHERE stall_ID = '".$_SESSION['stall_ID']."'".$filterCategory.$keyword." LIMIT ". $page .", 9;";
    $result = mysqli_query($conn, $sql) or die("<div class='col h5 text-center'>Something error</div>");
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) { 
            $image = $row['image'];
    ?>
    <div class="col-sm-6 col-md-4 p-2">
        <a href="#modal_<?= $row['ID'] ?>" data-toggle="modal" class="text-decoration-none text-reset">
            <div class="k-card card k-hover-shadow h-100">
                <div class="row no-gutters" style="position: relative;">
                    <div class="col-5">
                        <img style="width: 100%;height: 125px;" src="../images/<?= $stall_username; ?>/menu/<?= $image; ?>">
                    </div>
                    <div class="col-7">
                        <div class="card-body">
                            <div class="card-title h5 mb-0">
                                <?= $row['name']; ?>
                            </div>
                            <div class="card-text">
                                <small class="text-muted">
                                    <?= $row['category']; ?>
                                </small>
                            </div>
                        </div>
                    </div>
                    <span class="bg-dark text-white py-1 px-2" style="position: absolute;right: -5px;bottom: 10px;">RM <?=$row['price']; ?></span>
                    <?php if ($row['available'] == 1) { ?>
                        <div class="py-5 px-5" style="position: absolute;right: 0;top: 0;overflow: hidden;">
                            <span class="py-1 px-4 text-white" style="position: absolute;background-color: rgb(0, 255, 0);right: -25px;top: 11px;transform: rotate(45deg);overflow: hidden;">available</span>
                        </div>
                    <?php }else{ ?>
                        <div class="py-5 px-5" style="position: absolute;right: 0;top: 0;overflow: hidden;">
                            <span class="py-1 px-4 text-white" style="position: absolute;background-color: rgb(255, 0, 0);right: -25px;top: 5px;transform: rotate(45deg);overflow: hidden;">hidden</span>
                        </div>
                    <?php } ?>
                </div>
            </div>
        </a>
    </div>
    <?php 
    include 'menu_modal.php';
        } 
    } else if($keyword != ""){
    ?>
    <div class="col">
        <h4 class="text-center"><?php echo "No result for '".$_GET['k']."'"; ?></h4>
    </div>
    <?php
    }else if($filterCategory == ""){
    ?>
    <div class="col">
        <h4 class="text-center">
            Click the <i class="fas fa-plus"></i> button to add food
        </h4>
    </div>
    <?php
    }
    ?>
</div>
<?php
$sql = "SELECT 
food.name AS food_name, 
food.image AS image, 
food.price AS price
FROM food LEFT JOIN stall ON food.stall_ID = stall.ID WHERE stall_ID = '". $_SESSION['stall_ID'] ."'". $filterCategory.$keyword .";";
$result = mysqli_query($conn, $sql);
$count = mysqli_num_rows($result);
mysqli_close($conn);
$a = ceil($count / 9);
?>
<div class="col-12 mt-2">
    <ul class="pagination pagination-md">
        <?php for ($i=1; $i <= $a; $i++) { ?>
        <li class="page-item">
            <span class="page-link rounded-0 border-0 <?= $r = ($page == ($i * 9) - 9)? 'bg-dark text-white': 'text-dark' ?>" style="cursor: pointer;" data-page="<?= $i ?>"><?= $i ?></span>
        </li>
        <?php } ?>
    </ul>
</div>